...
function businesspro_optionsframework_add_admin() {
    global $query_string;

    $themename = businesspro_get_option('of_themename');
    $shortname = businesspro_get_option('of_shortname');
    ...
    if (isset($_REQUEST['page']) && $_REQUEST['page'] == 'optionsframework') {
        if (isset($_REQUEST['of_save']) && 'reset' == $_REQUEST['of_save']) {
            ...
            businesspro_reset_options($options, 'optionsframework');
            header("Location: admin.php?page=optionsframework&reset=true");
            die;
        }
    }
    $of_page = add_theme_page($themename, 'Theme Options', 'edit_theme_options', 'optionsframework', 'businesspro_optionsframework_options_page', 'div');

    // Add framework functionaily to the head individually
    add_action("admin_print_scripts-$of_page", 'businesspro_load_only');
}

add_action('admin_menu', 'businesspro_optionsframework_add_admin');
/* ----------------------------------------------------------------------------------- */
/* Options Framework Reset Function - of_reset_options */
/* ----------------------------------------------------------------------------------- */

function businesspro_reset_options($options, $page = '') {
    global $wpdb;
    $count = 0;
    ...
    if ($page == 'optionsframework') {
        businesspro_delete_option('of_options');
    }
}
...
function businesspro_optionsframework_options_page() {
    $options = businesspro_get_option('of_template');
    $themename = businesspro_get_option('of_themename');
    ?>
    ...
    <div class="wrap" id="of_container">
        <div id="of-popup-save" class="of-save-popup">
            <div class="of-save-save"><?php _e('Options Updated','business-pro'); ?></div>
        </div>
        <div id="of-popup-reset" class="of-save-popup">
            <div class="of-save-reset"><?php _e('Options Reset','business-pro'); ?></div>
        </div>
        <form action="" enctype="multipart/form-data" id="ofform">
    <?php wp_nonce_field('theme-update-option'); ?>
            <div id="header">
                <div class="logo">
                    <h2><?php _e('Business Pro Theme Options','business-pro'); ?></h2>
                </div>
                <a href="http://www.vathemes.com" target="_new">
    ...
    $return = businesspro_optionsframework_machine($options);
    ?>
            <div id="main">
                <div id="of-nav">
                    <ul>
    <?php echo $return[1] ?>
            ...
            <div class="save_bar_top">
                ...
                <input type="submit" value="<?php _e('Save All Changes','business-pro'); ?>" class="button-primary" />
        </form>
        <form action="<?php echo esc_attr($_SERVER['REQUEST_URI']) ?>" method="post" style="display:inline" id="ofform-reset">
            <span class="submit-footer-reset">
                <input name="reset" type="submit" value="<?php _e('Reset Options','business-pro'); ?>" class="button submit-button reset-button" onclick="return confirm('Click OK to reset. Any settings will be lost!');" />
...
function businesspro_load_only() {
    add_action('admin_head', 'businesspro_admin_head');
    ...
    function businesspro_admin_head() {
        ...
        foreach ($options as $option) {
            if ($option['type'] == 'color' OR $option['type'] == 'typography' OR $option['type'] == 'border') {
                if ($option['type'] == 'typography' OR $option['type'] == 'border') {
                    $option_id = $option['id'];
                    $temp_color = businesspro_get_option($option_id);
                    $option_id = $option['id'] . '_color';
                    $color = $temp_color['color'];
                } else {
                    $option_id = $option['id'];
                    $color = businesspro_get_option($option_id);
                }
                ?>
...
/* ----------------------------------------------------------------------------------- */
/* Ajax Save Action - businesspro_ajax_callback */
/* ----------------------------------------------------------------------------------- */
add_action('wp_ajax_of_ajax_post_action', 'businesspro_ajax_callback');
...
function businesspro_optionsframework_machine($options) {
    ...
    foreach ($options as $value) {
        ...
        switch ($value['type']) {
            ...
            case 'text':
                $val = $value['std'];
                $std = businesspro_get_option($value['id']);
                if ($std != "") {
                    $val = $std;
                }
            ...
            case 'select':
                ...
                $select_value = businesspro_get_option($value['id']);
            ...
            case "upload":
                $value['std'] = '';
                if (isset($value['std'])) {
                    $output .= businesspro_optionsframework_uploader_function($value['id'], $value['std'], null);
                }
                break;
            case "upload_min":

                $output .= businesspro_optionsframework_uploader_function($value['id'], $value['std'], 'min');

                break;
            case "color":
                $val = $value['std'];
                $stored = businesspro_get_option($value['id']);
                if ($stored != "") {
                    $val = $stored;
                }
            ...
            case "typography":
                ...
                $default = $value['std'];
                $typography_stored = businesspro_get_option($value['id']);
            ...
            case "border":
                ...
                $default = $value['std'];
                $border_stored = businesspro_get_option($value['id']);
        ...
        if (is_array($value['type'])) {
            foreach ($value['type'] as $array) {
                ...
                $id = $array['id'];
                $std = $array['std'];
                $saved_std = businesspro_get_option($id);
                if ($saved_std != $std) {
                    $std = $saved_std;
                }
                $meta = $array['meta'];
...
/* ----------------------------------------------------------------------------------- */
/* OptionsFramework Uploader - businesspro_optionsframework_uploader_function */
/* ----------------------------------------------------------------------------------- */
